...
\end{lstlisting}
    ...
    \ifans{The entries of $e_t$ corresponding to the padded tokens are set to -Inf by the $masked\_fill\_$ function. In the next step, we compute $\alpha_t$ by passing $e_t$ through a softmax, which assigns a 0 to large negative numbers (such as -Inf), effectively excluding padded tokens from being considered and attended to by our attention mechanism, as it should be.}
\end{parts}
...
\end{lstlisting}
    ...
    \ifans{\begin{subparts}
    \subpart[2] The dot product attention is computationally cheaper to obtain compared to multiplicative attention, as it presents a single matrix multiplication operation, compared to the two matrix multiplications of the multiplicative attention. However, this additional complexity introduced by \textbf{W} also allows the model to learn more complex interactions between key and query.
    \subpart[2] The non-linear activation function in the additive attention allows for the modeling of even more complex relationships. However, this comes at the cost of computational efficiency, as the additive attention does not benefit as hard from the most optimized matrix operation in modern frameworks - matrix multiplication.
    \end{subparts}}
\end{parts}
