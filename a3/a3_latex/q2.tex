...
\begin{parts}
    ...
    \begin{tabular}{ l | l | l | l}
    Stack & Buffer & New dependency & Transition \\ \hline
    [ROOT, attented, lectures] & [in, the, NLP, class] &  & \texttt{SHIFT} \\
    $[$ROOT, attented] & [in, the, NLP, class] & attended$\to$lectures & \texttt{RIGHT-ARC} \\
    $[$ROOT, attented, in] & [the, NLP, class] & & \texttt{SHIFT} \\
    $[$ROOT, attented, in, the] & [NLP, class] & & \texttt{SHIFT} \\
    $[$ROOT, attented, in, the, NLP] & [class] & & \texttt{SHIFT} \\
    $[$ROOT, attented, in, the, NLP, class] & [] & & \texttt{SHIFT} \\
    $[$ROOT, attented, in, the, class] & [] & class$\to$NLP & \texttt{LEFT-ARC} \\
    $[$ROOT, attented, in, class] & [] & class$\to$the & \texttt{LEFT-ARC} \\
    $[$ROOT, attented, class] & [] & class$\to$in & \texttt{LEFT-ARC} \\
    $[$ROOT, attented] & [] & attended$\to$class & \texttt{RIGHT-ARC} \\
    ...
    \begin{shaded}
    \begin{answer}
    $2n$ steps, as each of the $n$ words will have one step where it gets shifted from the buffer to our current stack, and one step where it is removed through either left or right-arc.
    \end{answer}
    \end{shaded}
...
\begin{shaded}
\begin{answer}
POS tags help disambiguate words with multiple meanings - i.e., if we have the "duck", which can mean both the animal or the act of bending/crouching, a POS can go a long way to help our parser make better informed decisions.
\end{answer}
\end{shaded}
\end{parts}
